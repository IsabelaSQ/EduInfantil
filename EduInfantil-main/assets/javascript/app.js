/* ============================================================
   SAFARI ALPHA — app.js
   Fluxo completo: 6 telas (inclui senha e sucesso) + validação + confete
   ============================================================ */
...
(function () {
  ...
  const state = {
    currentStep: 1,
    totalSteps: 5, // steps visíveis no progress (tela 5 é senha + tela 6 é sucesso)
    name: "",
    age: null,
    time: 30,
    lang: "simultaneous",
  ...
  function updateProgress() {
    const step = Math.min(state.currentStep, state.totalSteps);
    const pct = (step / state.totalSteps) * 100;
    progressFill.style.width = pct + "%";

    if (state.currentStep <= state.totalSteps) {
      stepLabel.textContent = `Passo ${state.currentStep} de ${state.totalSteps}`;
    } else {
  ...
  function updateFooter() {
    // Oculta footer somente na tela de sucesso
  ...
  function validateStep(step) {
    ...
    if (step === 5) {
      if (state.passwordSequence.length !== 3) {
        showError(err5, "Escolha 3 bichinhos para a senha.");
        return false;
      }
      hideError(err5);
      return true;
  ...
  btnNext.addEventListener("click", async () => {
    ...
    if (state.currentStep === 5) {
      const created = await criarExplorador();
      if (!created) return;
      buildSummary();
      goTo(6);
      launchConfetti();
  ...
  async function criarExplorador() {
    try {
      const res = await fetch('api.php', {
        ...
        body: JSON.stringify({
          ...
          nome: state.name || 'Explorador',
          emoji: state.characterEmoji || '🐘',
          senha: state.passwordSequence,
        }),
      });
      ...
      state.explorerId = data.id;
      return true;
    } catch (err) {
      console.error('Erro ao criar explorador:', err);
      openModal('❌', 'Falha ao criar explorador', 'Não foi possível salvar o explorador. Tente novamente.', null, 'OK');
      return false;
    }
  }

  function renderPasswordScreen() {
    state.passwordSequence = [];

    const slots = document.querySelectorAll('#password-slots .slot');
    slots.forEach(slot => {
      slot.textContent = '❓';
      slot.classList.remove('filled');
    });

    const grid = document.getElementById('password-animais-grid');
    if (!grid) return;
    grid.innerHTML = '';
    ...
    animals.forEach(animal => {
      ...
      btn.dataset.emoji = animal.emoji;
      btn.innerHTML = `
        <span class="animal-emoji">${animal.emoji}</span>
        <span class="animal-label">${animal.label}</span>
      `;
      btn.addEventListener('click', () => escolherSenhaAnimal(animal.emoji, btn));
      grid.appendChild(btn);
    });
  }

  function escolherSenhaAnimal(emoji, btn) {
    const selected = btn.classList.contains('selected');

    if (selected) {
      const index = state.passwordSequence.indexOf(emoji);
      if (index === -1) return;
      state.passwordSequence.splice(index, 1);
      btn.classList.remove('selected');
      atualizarSlotsSenha();
      return;
    }

    if (state.passwordSequence.length >= 3) return;

    state.passwordSequence.push(emoji);
    btn.classList.add('selected');
    atualizarSlotsSenha();
  }

  function atualizarSlotsSenha() {
    const slots = document.querySelectorAll('#password-slots .slot');
    slots.forEach((slot, index) => {
      const emoji = state.passwordSequence[index];
